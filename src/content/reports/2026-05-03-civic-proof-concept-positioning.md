...
「概念是否存在」與「概念是否被廣泛採用」必須分開處理。Anonymous credential 的學術文獻已存在 30 餘年，從 Chaum 1985<sup>2</sup>、Camenisch 與 Lysyanskaya 2001/2002 的 CL signature 構造<sup>3</sup>，到 BBS+ 的部署落地，技術鏈條完整。但 ISO 18013-5 mDL（mobile driving licence，2021）<sup>4</sup>預設 *未* 啟用 unlinkability across presentations；W3C VC Data Model 2.0（2025-05-15 Recommendation）<sup>5</sup>把 selective disclosure 列為可選 cryptosuite 而非 normative 必選；EUDI Wallet ARF v1.4（2024-2025）<sup>6</sup>對 unlinkability 條款的最終立場仍在 implementing regulations 層級擺盪。也就是說，工程能力存在，產品規格採用度有限。把「文獻存在」當作「規範條件已滿足」是常見的滑移。
...
6. European Commission. (2024–2025). *European Digital Identity Wallet Architecture and Reference Framework v1.4*. EUDI ARF. 來源等級 A。
...
23. NIST. (2024). *Digital Identity Guidelines* (Special Publication 800-63-4); EU. *eIDAS Regulation* (Regulation (EU) No 910/2014)；ISO/IEC 29115 entity authentication assurance framework. 來源等級 A。
