...
- **OpenFun 適合做批次資料層，新 API 已可搜尋與查詢受保護資料，但預算仍走 bulk 檔。** 實際 Token 可讀取 17,388 筆機關代碼，欄位搜尋與精確篩選均成功；兩份預算資料集的 `records` 與 `agg` 則都回傳 HTTP 400。22 縣市靜態檔實測得到 21 份可讀工作計畫 CSV，共 45,505 列，只有 11 個縣市的詳細歲出 JSON 實際含有計畫文字。
- **Twinkle 已從搜尋測試進入實際資料列驗收，但可採用範圍仍小。** 22 縣市的 MCP 呼叫全部成功，8 個全縣市候選都完成 `query_rows` 實讀；只有新北市與嘉義市的回傳總額和官方 115 年歲出完全相符，目前用來補充機關別明細。其餘 6 個因舊年度或總額不符而排除。
...
2026 年 7 月 23 日，我使用正式 OpenFun Token 重跑搜尋、records、欄位搜尋、精確篩選與聚合。未帶 Token 查詢 `tw.gov.dgpa~ref~gov-org` records 時回傳 401，帶入 Token 後回傳 200，資料集共有 17,388 筆與 15 個 schema 欄位。`q[機關名稱]=臺北市政府` 回傳 95 筆，`機關層級=1` 精確篩選回傳 9 筆，證明 Token、全文欄位搜尋與等值篩選都能實際工作。

預算路徑的結果不同。匿名與認證搜尋都能找到 `tw.openfun~bulk~budget-local` 和 `tw.openfun~bulk~budget-detail`，兩種身分看到的預算資料集相同。兩份資料集都屬於 bulk 類型，呼叫 records 時回傳 `This dataset does not support record queries`，呼叫 agg 時回傳 `This dataset does not support aggregation`。因此「OpenFun 有通用 API」和「地方預算可用通用 API 逐筆查詢」必須分開描述。目前 OpenBook 的預算數據仍直接下載公開 CSV／JSON，Token 用於驗證平臺能力，沒有供應網站上的預算數字。

測試也找到一個文件與正式行為的落差。機關代碼 skill.md 提供 `group_by=機關層級` 的 agg 範例，正式 API 卻回傳 HTTP 400，訊息為該欄位必須在 schema 標記 `filter:true`。records 的 `機關層級=1` 精確篩選仍可成功，顯示問題集中在聚合 schema 或端點驗證。這個案例適合加入自動化契約測試，也應回報文件或 schema 維護端。

完整與授權資料集目前為邀請制封測，需要帳號與群組授權；有使用需求可聯絡歐噴資料庫團隊。這項存取條件不影響上述預算 bulk 檔的公開性，地方預算 CSV／JSON 仍可免 Token 下載。
...
OpenFun 最適合放在 OpenBook 的批次資料層。每天或每次發布版先下載、存雜湊、驗 schema、對官方總額，再把通過檢查的資料寫入共同模型。搜尋 API 和 skill.md 可用來發現資料集及取得欄位指南；預算 records／agg 尚未支援前，不宜把它包裝成互動式預算 API。網站應顯示 OpenFun 整理責任及官方原始來源，不能讓民間正規化檔取代證據鏈。
...
第二輪驗收進一步對這 8 個候選呼叫 `query_rows`，全部成功取得實際資料列。新北市 dataset `177273` 回傳 63 列，總計 2,605.632 億元，和官方 115 年歲出完全相符；嘉義市 dataset `52350` 回傳 13 列，總計 211.25 億元，也完全相符。OpenBook 因此開始使用兩者的機關別明細，但官方共同表仍是正式總額來源。嘉義市來源未明示年度，網站另外揭露目前是以總額完全相符作為判斷依據。

其餘候選未通過。桃園、臺南、高雄與南投的 Twinkle 回傳總額分別和官方基準相差約 46.7%、20.8%、14.3% 與 4.1%；臺中資料列日期是 2019 年，花蓮金額欄明示民國 107 年。這些結果證明 MCP 能讀資料，不代表讀到的資料適合目前年度。
...
內容品質仍需第二道驗收。新竹縣最佳命中的 Big5 CSV 欄位出現亂碼；臺中全市歲出候選只有 2 列 2019 年資料；部分資料的 `is_normalised`、schema 與快取狀態也不完全一致。Twinkle 的價值是讓 AI agent 統一「找資料、看 schema、查資料、物化檔案」的操作，但正式使用必須同時通過 dataset ID 白名單、官方來源 URL、欄位型別、年度與總額對帳。
...
| AI 操作 | 需自行下載與解析 | `skill.md` 與搜尋 API 友善，預算本體仍需讀 bulk | 原生 MCP，工具探索最方便 |
| 實際讀列 | 依各官方平臺 | 受保護 records 可用，預算 bulk 不支援 records | `query_rows` 可用，8 個全縣市候選已實讀 |
| 聚合 | 依工作簿自行計算 | 預算 bulk 不支援 agg，機關代碼文件範例也暫時失敗 | 由使用端在讀列後計算 |
| 批次重建 | 適合，需城市轉接器 | 最方便，免 token 靜態檔 | 可做，但不宜讓 API key 成為靜態建置條件 |
| 主要風險 | 格式分散、連結與 schema 改版 | 空檔、404、年份不齊、單位差異 | 搜尋誤命中、編碼、型別、文件與正式工具差異 |

如果問題是「現在誰比較好用」，答案取決於工作。OpenFun 對預算研究者比較好用，因為預算專用 skill.md、跨年度 bulk、工作計畫、計畫內容與用途別明細更接近 OpenBook 的資料模型，21 個縣市可直接批次載入。Twinkle 對需要跨目錄探索的 AI agent 比較好用，因為 MCP 的搜尋、metadata 與 `query_rows` 操作一致，不必先處理每個官方網站的下載方式。

若目標是支撐 OpenBook 正式頁面，OpenFun 目前的整體價值較高，但主要來自公開 bulk 與預算語意，不是新版 records API。若目標是自然語言找資料並快速試讀，Twinkle 的互動體驗較完整。兩者的共同限制是不能自行決定正式數字，OpenFun 必須處理缺檔與年度，Twinkle 必須處理誤命中與總額不符。最穩健的配置仍是官方資料負責原始證據，OpenFun 負責批次正規化與深度，Twinkle 負責發現及選擇性讀列，再由共同驗證層把結果接起來。
...
| OpenFun REST | 民間 | REST、skill.md | Bearer；完整權限邀請制封測 | 搜尋、受保護資料查列 | 預算 bulk 不支援 records／agg，另有聚合文件落差 |
| Twinkle Hub | 民間 | MCP | Bearer | 通用 agent 資料工具 | 22 市呼叫成功，8 市實讀，只有 2 市通過對帳 |
...
3. Twinkle 使用 dataset ID 與欄位規則白名單。語意搜尋只產生候選，`query_rows` 實讀後仍須和官方年度及總額對帳；通過者可補機關別明細，不能取代官方總額。
...
[Open-Book repo](https://github.com/mashbean/Open-Book) 的全台版已有兩條可重建管線。`npm run refresh:interfaces` 先用 OpenFun Token 測試搜尋、records、篩選、聚合與預算端點，再逐縣市讀取公開 bulk 檔並連接 Twinkle 正式 MCP，產生不含金鑰的接口驗收摘要。`npm run refresh:national` 下載主計總處 XLSX，驗證 22 縣市完整性、八類政事別、經資門與收支平衡，再把雙接口結果合併進前端 `cities.json`。

頁面預設進入全台比較表，上方提供全台總覽與 22 個可獨立分享的縣市分頁。比較表把官方 115 年總預算、OpenFun 資料深度、TwinkleAI 實際採用狀態與查證問題放在同一列。每個縣市頁都呈現八類政事別、融資平衡式、OpenFun 工作計畫，以及 TwinkleAI 的 dataset ID、schema、實讀列數、回傳總額、官方差額與排除理由。新北市和嘉義市另呈現通過對帳的機關別明細；其他縣市則明示「已讀列但未通過」或「只有搜尋結果」。臺北額外的官方 XML 明細也收進單一臺北頁。

網站同時把三種資料明確標成不同責任。官方資料是可被追責的證據底座，OpenFun 明確拆成「API 目錄」與「公開預算 bulk」兩條路徑，TwinkleAI 則分成搜尋、讀列與對帳三個階段。全台頁新增逐項能力比較與邀請制封測註記，公開 OpenFun 的 17,388 筆 Token 驗證、預算 records／agg 不支援、Twinkle 的「8 個實讀、2 個採用」；完整 CSV 也標出 OpenFun 來源路徑及 Twinkle 回傳總額與官方差額。
...
台灣已經具備全台 OpenBook 的最低資料條件。主計總處的地方總預算彙編提供 22 縣市共同底圖，地方平台提供不同深度的正式來源。OpenFun 是目前最實用的批次預算整理層，21 市可讀 CSV 與 11 市非空詳細 JSON 已足以支援相當多工作計畫分析；新版 API 的搜尋與受保護 records 也已能實際使用，但預算 bulk 尚未進入通用 records／agg。Twinkle Hub 是目前最直接的臺灣資料 MCP，搜尋、metadata 與 `query_rows` 都能運作；本次只有 2 個縣市通過正式對帳，顯示它已能補明細，但仍不能單獨決定資料來源。

目前的主要缺口是一份穩定的跨縣市預算語意契約。資料發布者、整理者、視覺化網站與 AI 服務目前各自解決一段問題。全台 OpenBook 的工作，是把這些段落接起來，又保留每筆數字的來源、年度、階段與責任。

接下來最值得做的工程有三條。第一條用新北、桃園、臺中、臺南與高雄完成官方結構化城市轉接器。第二條用 OpenFun 已有非空詳細 JSON 的 11 個縣市完成計畫文字與用途別搜尋。第三條擴充 Twinkle dataset ID 與欄位規則白名單，保留目前 2 個通過案例，逐一修正另外 6 個候選的年度與範圍。官方若未來提供預算 API 或 MCP，也能接到同一個來源登錄與驗證層，不必重做居民介面。
