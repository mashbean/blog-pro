...
去年某個下午我手癢去翻 W3C DID Specification Registries，本來只是想找一條漂亮的 method 直接套到一個小實驗，結果讀著讀著有點愣住。整份清單上掛著 did:btcr、did:ethr、did:ion、did:sov，每一條 method 都是從某條公共鏈長出來的；但同年我去盤點各國政府數位身分計畫，能找到把 trust root 真的放在無許可公共鏈上的案例，扣著手指算只有阿根廷布宜諾斯艾利斯的 QuarkID 跑在 ZKsync 上，勉強算有點稜角<sup>3</sup>。

剩下的歐盟 EBSI、ISO mDL、新加坡 Singpass、印度 Aadhaar、巴西 gov.br、日本マイナンバー数字認証アプリ（My Number 數位認證 App）、台灣 FidO 與數位皮夾，幾乎清一色選了聯盟鏈、政府 PKI，或乾脆完全無鏈。

這個落差我看了挺久。技術社群當年那一批熱血十大原則，跟十年後的部署清單對不上的差距為什麼會這麼大？我一開始的猜測是某種陰謀論（某些大公司在標準制定時偷偷把公共鏈擠出去 XD），但讀完規格、判決、ARF 文件，這個劇本撐不太住。
...
把目前可公開蒐集的政府級身分計畫攤開排排站，trust root 在無許可公共鏈上的案例極稀。

歐盟層級的 EBSI 是聯盟鏈，使用 Hyperledger Besu 或 Quorum 變體，由會員國節點維護<sup>17</sup>。ISO/IEC 18013-5:2021 mDL 是 PKI 加上設備本地的設計，完全無鏈；AAMVA 美國駕照數位化指南沿用此架構<sup>18</sup><sup>19</sup>。新加坡 Singpass 以 NRIC 為核心、搭配政府 PKI<sup>20</sup>。印度 Aadhaar 是中央化生物特徵資料庫加 API<sup>21</sup>。巴西 gov.br 採聯邦 PKI<sup>22</sup>。日本マイナンバー数字認証アプリ 2023 年推出，仍以 PKI 為信任根<sup>23</sup>。台灣 FidO 與數位皮夾以自然人憑證、內政部憑證管理中心 PKI 為信任根；數位發展部相關規劃文件指向同一條 PKI 路線，具體文件 ID、年度預算與 DID method 規劃細節在公開層仍偏薄<sup>24</sup>。
...
例外與邊角實證也得明白標出。阿根廷 Buenos Aires 與 Extrimian、Matter Labs 合作的 QuarkID，2024 年部署在 ZKsync L2，是政府級 DID 真正落地在公共鏈的稀有實例。第一階段已開放給布宜諾斯艾利斯市民日常使用，實際採用規模、長期維運、跨域互通仍待後續驗證<sup>3</sup>。不丹 NDI 在 Druk Holding & Investments 主導下於 2023 年上線，公開資訊指向以 Hyperledger Indy / Aries 框架為底層，確切版本與許可模型尚未在公開層完整揭露；坊間把它誤傳為「公共鏈身分典範」並不準確<sup>28</sup>。

代幣化身分這條路線是另一個值得分開看的範疇。Cheqd 把 VC 撤銷與 trust registry 與代幣經濟綁在一起<sup>29</sup>；Polygon ID、KILT、Atala PRISM 等也各有民間試點與企業採用<sup>30</sup><sup>31</sup><sup>32</sup>；Worldcoin 把生物特徵唯一性與代幣結合，遭遇西班牙、葡萄牙、肯亞、香港等地的隱私調查或暫停<sup>33</sup>。這些案例有個共同點，政府與大型受監管金融機構直接把 trust root 移交給代幣化基礎設施的實證，目前仍接近於零。把它們一筆帶過為「失敗」並不準確。它們是另一條路線，目前在邊角試點與代幣經濟驅動的場域裡持續演化。

台灣的本地實證密度有限。公部門 FidO 與數位皮夾走 PKI；民間端 Numbers Protocol 把影像與事件存證寫到 IPFS 與鏈上，採用方為合作媒體與 NGO，不直接承擔身分簽發角色，比較像建設性的時間戳功能<sup>34</sup>。FAB DAO 2021 年起以鏈上投票實驗、NFT 募款形式運作，屬公民倡議與 DAO 治理範疇，未承擔身分根<sup>35</sup>。把這些民間試點跟「政府身分基建」混為一談是常見誤讀。更準確的描述應該是民間鏈上實踐存在但邊緣，公部門信任根仍在 PKI。

把以上案例攤平排在一起看，去鏈化是一個跨地理、跨制度的均衡，而非歐洲特有現象。公共鏈在政府級身分基建裡保留的是邊角位置，並非主流位置。
...
第四個位置是「高自治、低監管脈絡下的 trust root」。阿根廷 QuarkID 是這條路徑的代表<sup>3</sup>。在規模較小、法規相對寬鬆、政府願意承擔技術前緣風險的脈絡裡，公共 L2 仍可能直接承擔 trust root。把這當作普遍解會誤判；當作特定邊界條件下的可行解，比較準確。邊界條件包含幾項要素，包含法規面寬鬆、政權延續性可預期、L2 finality 與成本已壓到可接受區間、跨域互通在地理範圍內可控。

第五個位置是邊角實驗的試錯空間。Cheqd、Polygon ID、KILT、Atala PRISM、Worldcoin、不丹 NDI 各自在不同假設下做實驗<sup>28</sup><sup>29</sup><sup>30</sup><sup>31</sup><sup>32</sup><sup>33</sup>。即使其中多數短期內不會被法規導向的政府部署採納，這個試錯本身對 SSI 整體規格演化有價值。L2/ZK 撤銷模型、token-driven trust registry、生物特徵唯一性協定、許可式 SSI 與公共鏈間的橋接，這些議題都需要持續跑出實證。這層試錯像皮克敏升等，每一隻死掉的小皮克敏都把後面那一群往前推了一點點。
...
不丹 NDI 確切技術棧，公開資訊指向 Hyperledger Indy / Aries 框架，但版本、許可模型、是否有任何鏈上錨定設計，目前都還沒在公開層留下清楚紀錄<sup>28</sup>。台灣 FidO 與數位皮夾的具體文件 ID、年度預算金額、DID method 規劃進度，公開資訊密度有限<sup>24</sup>。本地實證在這兩處都偏薄，需要進一步資料補強。
...
回頭看 2018 年第一次讀 Christopher Allen 那十大原則時的興奮，跟 2026 年看著各國 wallet 都是 PKI 的感覺，落差很大。當年覺得公共鏈會把整片身分基建吃下來；現在覺得它只是換了一個身位站在後面。對於還在台灣這邊看數位皮夾規劃的我來說，這個觀察反而給了一點底氣，公共鏈在身分基建還能做事，題目只是長得跟十年前的願景不太一樣了。

下一個十年呢？等 L2 finality 跌破一秒、可大量批次撤銷的鏈上資料結構出現之後，今天這份清單裡哪幾個位置會被擠出去、哪幾個會擴張，老實說我也還沒答案。
...
24. 數位發展部 / 內政部憑證管理中心. https://moda.gov.tw/、https://moica.nat.gov.tw/ （查閱日期 2026-04-30）。來源等級 A/B；具體文件 ID、年度預算（NTD）與 DID method 規劃細節待查核。
...
28. Bhutan NDI（Druk Holding & Investments）. https://www.bhutanndi.com/ （查閱日期 2026-04-30）。來源等級 B；確切技術棧版本與許可模型待查核。
