...
_這是「有備而來：理想的數位皮夾開發報告」第四篇。本文依 2026 年 9 月 4 日至 5 日的原始碼、Cloudflare 部署與實機操作撰寫。所有秒數來自 App 與驗證器自動記錄的單機樣本，數量很少，只報原始值與中位數，不稱為 p95。測試用真實卡片時只記錄階段、結果、耗時與錯誤類型；本文與程式碼都不收錄出生日期、統一編號或任何揭露欄位。_
...
<figure class="phone-shot">
  ...
  <figcaption><em>圖一。有備而來「使用」分頁的零知識證明區。舊的 MOICA 持有證明已從選單移除，只留年齡述詞證明的建立與查驗兩列，避免兩種零知識證明並列造成混淆。</em></figcaption>
</figure>
...
1. 查驗網站 [verifier.mashbean.net/zkp](https://verifier.mashbean.net/zkp) 新增一頁，能建立一次性的零知識年齡查驗，並顯示每一段耗時。
...
| 層次 | 專案 | 在這裡的角色 |
| --- | --- | --- |
| 電路與證明系統 | [ethereum/zkID](https://github.com/ethereum/zkID)（OpenAC，commit `b395e09`） | `jwt_2k` 與 `show` 兩個 Circom 電路。前者驗證發卡者的 ES256 簽章並承諾隱藏的欄位，後者證明述詞並綁定查驗方 nonce |
...
| 驗證端原生服務 | `openac-age-verifier`（Rust、axum） | 載入兩把驗證金鑰，執行 `verify_linked`，比對 6 加 156 個公開值 |
...
| 參考實作 | PSE 的 go-zkid-verifier、openac-rsa-x509-swift、moica-revocation-smt | 第一代 MOICA 持有證明的路線；本次未採用，理由見決策鏈 |
...
<figure class="phone-shot">
  ...
  <figcaption><em>圖三。同意畫面說明四件事，包括對方問什麼、為了什麼、用哪張卡、證明會送去哪裡。最後一項是與面對面藍牙流程唯一的差別，因此額外標示。</em></figcaption>
</figure>
...
<figure class="phone-shot">
  ...
  <figcaption><em>圖四。手機端收到網站的判定與秒數。這一筆是 Prepare 快取命中的結果，建證耗時 2.7 秒。</em></figcaption>
</figure>
...
第二，Worker 呼叫容器的方式。`@cloudflare/containers` 的 `containerFetch(url, init, port)` 把 `RequestInit` 當位置參數傳過 Durable Object 的 RPC 邊界，body 與 headers 無法送達。容器本身健康，Worker 卻收到 502。改為建立一個 `Request` 交給 stub 的 `fetch`，由 Container 基底類別代理到預設埠，問題解決。
...
| 項目 | 方案與額度 | 這次的實際 |
| --- | --- | --- |
| Cloudflare Workers Paid | 每月 5 美元，含 Durable Objects | 既有方案，示範站原已部署於此 |
| Containers 免費額度 | 每月 25 GiB-hours 記憶體、375 vCPU-minutes、200 GB-hours 磁碟 | basic 醒著時計 1 GiB，約 25 小時免費；閒置 10 分鐘休眠，休眠不計費 |
| 超額費率 | 記憶體 0.0000025 美元/GiB-s、CPU 0.000020 美元/vCPU-s | 測試量沒有超額；basic 全天不休眠約每月多 7 美元，standard-1 約 27 美元 |
| Mac 隧道路線 | 0 美元 | 需要一台開機的機器，網址會變 |
| GitHub release 託管 80 MB XCFramework 與金鑰 | 0 美元 | 每支手機第一次建證下載約 76 MB 素材 |
| 開發時間 | 不計價 | 本報告涵蓋的工作約兩個工作天 |
...
<figure>
  ...
  <figcaption><em>圖六。手機端的診斷頁做同樣的並排，數字來自 App 的單調時鐘。政府卡片那一列的零知識「尚未量測」，原因見誠實邊界。</em></figcaption>
</figure>
...
<figure>
  ...
  <figcaption><em>圖七。切換到容器後的第一份真證明。判定正確，後端驗證 24.5 秒，是容器剛啟動加上 1/4 vCPU 的結果。</em></figcaption>
</figure>
...
<figure>
  ...
  <figcaption><em>圖八。緊接著的第二份。驗證降到 5 秒，這是 basic 規格的穩態。瓶頸在 vCPU，記憶體並非限制。</em></figcaption>
</figure>
...
| --- | --- | --- | --- |
| Prepare 證明 | 2,102 到 2,987 ms（另 key setup 3,254 到 3,499 ms） | 首次含 witnesscalc 11,478 ms；無快取 7,586 ms | 不適用 |
| Prepare 重新遮罩 | 856 到 884 ms | 約 2,200 ms | 不適用 |
| Show 證明加遮罩 | 115 到 129 ms | 467 到 542 ms | 不適用 |
| Prepare 驗證 | 137 到 151 ms | 不適用 | Mac 157 到 200 ms；容器 4,973 ms |
...
<figure class="phone-shot">
  ...
  <figcaption><em>圖九。皮夾裡的三種來源。目前只有國民身分證（MyData 自發）能做年齡證明，駕照電子卡與門號電子卡都沒有揭露出生日期，電路無從證明。</em></figcaption>
</figure>

- **能做年齡證明的政府卡片目前是零張。** 實測的公路局駕照電子卡沒有 `roc_birthday` 欄位，門號電子卡也沒有。限制來自發卡端未把該欄位放進卡片，與有備而來的實作無關。診斷頁上政府卡片那一列的「尚未量測」即為此意。
- **自發身分證的年齡證明沒有政府背書。** 它隱藏的是持卡人從 MyData 資料自行簽署的出生日期。頁面與 App 都標示「自發、非政府背書」，這行字不能移除。
- **證明包帶著發卡者 did:key。** 政府卡片是發卡機關的金鑰；自發身分證是持卡人自己的每卡金鑰，是一個穩定假名。允許清單因此存在，證明只能送往已知的站台。
- **容器的速度。** basic 規格暖機 5 秒、冷啟動 24.5 秒，後者接近 Worker 對後端 60 秒的逾時。建立 session 時已預先喚醒容器，手機建證的數秒是暖機窗口，但不足以覆蓋 24.5 秒的冷啟動，且 1/4 vCPU 是下限。
- **樣本很少。** 每個數字來自一到三次實機執行，只能說明量級，不能當作效能規格。
- **未完成的事項。** 未對 OpenID4VP 或任何零知識規格執行 conformance；未實作零揭露的「持有證明」電路（需要修改電路輸入並重建 XCFramework）；quick tunnel 路線已退場；容器同時驗證上限 2 的目的是控制記憶體，與吞吐量無關。
