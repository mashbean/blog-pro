...
除了儲存格式，規劃階段還評估過兩條以零知識證明直接為 MyData 文件建立信任根的路。兩條都在 2026 年 8 月被排除。密碼學本身沒有問題，卡住的地方是 MyData 交付資料的方式。

**zkPDF 的前提是文件帶簽章。** 隱私與規模探索小組（PSE）的 zkpdf 能在電路裡驗證 PDF 內嵌的 PKCS#7 RSA-SHA256 簽章，再證明某一頁出現某段文字，持有人不必交出整份文件。這條路一度是首選，8 月 9 日的分岔表寫著「有簽章就走 zkpdf」。同一天以真實帳號下載的戶政 PDF 量出十四個簽章標記全為零之後，這條路關閉。沒有簽章的 PDF 放進 zkpdf，能證明的只有「這份檔案含有這段文字」，任何人用文書軟體都能做出同樣的檔案，證明不了機關說過這些話。
...
3. **TLS 版本。** 2026 年 9 月實測，mydata.nat.gov.tw 預設協商 TLS 1.3，仍接受 TLS 1.2。TLSNotary 文件寫明目前支援 TLS 1.2，1.3 在路線圖上。今天能接，平臺一次安全升級就會斷。
